...
class Site extends CI_Controller{
    ...
    public function getvideo($id)
    {
     ...
     public function ajax_produk($no)
    {
    ...
    {
        $email = $this->input->post('email');
       $cek = $this->Admin_model->sum_data_param('email',$email,'tbl_homeowner');
        if($cek == 0)
        {
            echo 0;
        } else if($cek > 0) {
    ...
    public function reset_password_homeowner($email){
        ...
        if($update)
        {
            ...
            if($this->email->send())
            {
               print "<script type=\"text/javascript\">alert('Password Anda berhasil di reset. Mohon periksa email Anda.');window.location = '/site/home-owner';</script>";
            }
        }
    }
