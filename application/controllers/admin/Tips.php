...
/**
 * Name : 3motion.cms base controller.
 *
 * @version 1.0.0
 *
 * @author : Toni Wibowo
...
class Tips extends MX_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->library(array('output_view','ion_auth'));
    ...
    public function index()
    {
		...
		//$crud->where('publish','Y');
		//$crud->order_by('product_id','DESC');
        ...
        if ($table->uploads != '') {
            ...
            foreach ($fields_upload as $field_upload) {
                $crud->set_field_upload($field_upload, 'images/articles/');
            }
        }

		//$crud->field_type('date', 'hidden', date('Ymd'));
    $user = $this->ion_auth->user()->row();
    $crud->field_type('post', 'hidden', $user->username);
        ...
        "path_to_directory"       =>'images/articles/',
